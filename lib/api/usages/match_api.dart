...
class MatchApi {
  ...
  static Future<int> getLastMatchId(String team) async {
    var api = DataFactory.getDataGetter();
    var result = await api.get("match/$team");
    return (result.body as Map<String, dynamic>)["id"];
  }
  ...
  static Future<List<MatchHistory>> getMatchHistoryById(int matchId) async {
    var api = DataFactory.getDataGetter();
    var result = await api.get("match/history/$matchId");
    List<MatchHistory> history = [];
    if (result.statusCode == 200) {
      for (var event in result.body as List<Map<String, dynamic>>) {
        MatchHistory historyEvent = MatchHistory();
        historyEvent.fromJson(event);
  ...
  static Map<String, List<Positions>> positions = {
    "GK": [Positions.goalkeeper],
    "DEF": [
      Positions.leftBack,
      Positions.centerBack,
      Positions.rightBack,
    ],
    "MID": [
      Positions.defensiveMidfielder,
      Positions.centralMidfielder,
      Positions.attackingMidfielder,
    ],
    "ATT": [
      Positions.leftWinger,
      Positions.striker,
      Positions.rightWinger,
    ],
    "SUB": [Positions.substitute],
  };
